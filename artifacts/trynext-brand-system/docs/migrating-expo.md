...
Read `artifacts/trynext-brand-system/docs/AGENTS.md` and
`artifacts/trynext-brand-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/trynext-brand-system/lib/native-theme` and
  `@workspace/trynext-brand-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/trynext-brand-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/trynext-brand-system` and retains only
