# Migrating Expo UI to Trynext Atelier Commerce

Read `artifacts/trynext-atelier-commerce/docs/AGENTS.md` and
`artifacts/trynext-atelier-commerce/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/trynext-atelier-commerce/lib/native-theme` and
  `@workspace/trynext-atelier-commerce/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/trynext-atelier-commerce/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/trynext-atelier-commerce` and retains only
