...
**How to apply:** `lib/api.ts`'s `getBaseUrl()` and `_layout.tsx`'s `setBaseUrl()` detect that stale domain at runtime and transparently replace it with the correct current domain (`trynext.pages.dev`), so old builds self-heal without needing a resubmission.
